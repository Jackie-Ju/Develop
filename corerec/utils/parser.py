...
def parse_args():
    ...
    parser.add_argument('--fraction', type=float, default=0.3, help="coreset size")
    parser.add_argument('--method', type=str, default="KCore_repeat1_0.5", help="the method used to select coreset")
    parser.add_argument('--model', type=str, default="lightgcn", help="the method used to select coreset")
...
args = parse_args()
global_path = '~/projects/Develop/corerec/config/global_config.py'
config_file = f'~/projects/Develop/corerec/config/{args.model.strip()}_config.py'
cfg = load_config_data(global_path, config_file)
cfg.train_args.device = args.device.strip()
cfg.core_args.type = args.method.strip()
...
cfg.core_args.model_dict_path = os.path.join(cfg.early_stopping.root,
             f"{cfg.method_type}/{cfg.method_type}-{cfg.model.architecture}-"
             f"{cfg.dataset.name}-{cfg.core_args.fraction}.pth")
